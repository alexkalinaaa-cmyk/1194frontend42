...
(function(){
  ...
  // Settings (MSAL-enabled)
  const S = { root: $("#settings-overlay"), save: $("#settings-save"), close: $("#settings-close"),
    occ: $("#set-occupation"), phone: $("#set-phone"), theme: $("#set-theme"), 
    authBtn: $("#auth-action-btn"), authSignoutBtn: $("#auth-action-btn-signout") };
  const KSET = "JL_settings_v3"; // Updated version for MSAL
    ...
    on($("#btn-settings"),"click",function(){
      ...
      if (window.IndexedStorage && window.IndexedStorage.Storage) {
        window.IndexedStorage.Storage.getItem(KSET).then(result => {
          try{ 
            const v=JSON.parse(result||"null"); 
            if(v){ 
              S.occ.value=v.occ||""; 
      ...
      if (window.currentUser) {
        window.updateUIForSignedInUser();
      } else {
        window.updateUIForSignedOutUser();
      }
    });
  on(S.close,"click",function(){ S.root.setAttribute("hidden",""); });
  
  // Auth button click handlers
  on(S.authBtn,"click", async function(){
    // Sign in button
    await window.signIn();
  });
  
  on(S.authSignoutBtn,"click", async function(){
    // Sign out button
    ...
    controlsToManage.forEach(control => {
      ...
      if(element) {
        ...
        if(!hasReportId) {
          ...
          if(control.type === "input" && control.id === "job-name") {
            element.placeholder = "Enter job name (required)";
          } else if(control.type === "input" && control.id === "job-code") {
            element.placeholder = "Enter job code (required)";
          } else if(control.type === "input" && control.id === "location-field") {
