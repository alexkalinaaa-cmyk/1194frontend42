...
(function(){
  ...
  function loadCurrentPlan() {
    ...
    img.onerror = function() {
      console.warn('Failed to load floor plan image, trying to restore from stored data...');
      
      // Try to restore blob URL and retry
      if (plan.originalSrc || plan.src) {
        const fallbackSrc = plan.originalSrc || plan.src;
        if (fallbackSrc !== img.src) {
          console.log('Retrying with fallback image source');
          img.src = fallbackSrc;
          return;
        }
      }
      
      console.error('All image sources failed for floor plan:', plan.id);
    };
    ...
    console.log('Loading floor plan image from:', imageSource.substring(0, 50) + '...');
    img.src = imageSource;
  }
