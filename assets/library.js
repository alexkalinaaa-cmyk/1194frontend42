(function(){
  ...
  async function uploadToSharePoint(pdf) {
    ...
    try {
      ...
      // Collect required metadata from MSAL authentication
      const userData = window.getCurrentUserData();
      const author = userData?.name || 'Unknown Author';
  ...
  const loadJobName = async (jid) => await read(K_JOBNAME + jid + "_v1", "");
  const saveJobName = async (jid, name) => await write(K_JOBNAME + jid + "_v1", name || "");
  
  // Helper function to check if PDFs exist for a report ID
  const checkExistingPDFs = async (jid) => {
    const allPDFs = await loadPDFs();
    return allPDFs.filter(pdf => pdf.reportId === jid);
  };
  
  // Helper function to load report-specific data (job name, job code, location) into UI
  const loadReportDataToUI = async (jid) => {
    // Load Job Name for this Report ID
    const jobNameInput = document.getElementById('job-name');
    if (jobNameInput) {
      jobNameInput.value = await loadJobName(jid || "");
    }
  ...
  async function ensureCur(autoCreate = true){
    ...
    if(!id && autoCreate){
      ...
      // Load report-specific data for the newly created report (will be empty but clears any existing data)
      await loadReportDataToUI(reportId);
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      attachedJobs.forEach(function(jid){
        ...
        childBtn.addEventListener("click", async function(){
          ...
          // Load report-specific data (job code, location) for this Report ID
          await loadReportDataToUI(jid);
  ...
  async function validateRequiredForExport(jid) {
    ...
    // Check MSAL authentication (Name and Email)
    try {
      const userData = window.getCurrentUserData();
      if (!userData) {
        missingFields.push("Microsoft Account Sign-In");
      } else {
        if (!userData.name || !userData.name.trim()) {
          missingFields.push("Name (Sign-In Required)");
        }
        if (!userData.email || !userData.email.trim()) {
          missingFields.push("Email (Sign-In Required)");
        }
      }
    } catch (error) {
      console.warn('Error checking MSAL authentication:', error);
      missingFields.push("Microsoft Account Sign-In");
    }
  ...
  async function showPDFFilenameModal(jid, reportNameEntry, items) {
        ...
        <div class="content">
          <div style="margin-bottom: 16px; padding: 12px; background: #1f2937; border-radius: 8px;">
            ...
            <div style="margin-top: 8px; font-size: 14px; color: #94a3b8;">
              ...
              <div>Job Name: <strong id="pdf-preview-job-name">Loading...</strong></div>
              <div>Job Code: <strong id="pdf-preview-job-code">Loading...</strong></div>
    ...
    // Update job name and job code preview in modal
    const jobNamePreview = modal.querySelector('#pdf-preview-job-name');
    const jobCodePreview = modal.querySelector('#pdf-preview-job-code');
    const jobNameValue = document.getElementById('job-name')?.value?.trim() || '';
    const jobCodeValue = document.getElementById('job-code')?.value?.trim() || '';
    
    jobNamePreview.textContent = jobNameValue || '';
    jobCodePreview.textContent = jobCodeValue || '';
    
    // Function to generate filename based on job code, report name, and date
    async function generateFilename() {
      const jobNameFromField = document.getElementById('job-name')?.value?.trim() || '';
      ...
      // Add job name (with underscores for spaces)
      if (jobNameFromField) {
        filename += jobNameFromField.replace(/\s+/g, '_') + '_';
      } else if (reportName) {
        // Fallback to report name if no job name is provided
        filename += reportName.replace(/\s+/g, '_') + '_';
      }
    ...
    modal.querySelector('#pdf-generate-btn').addEventListener('click', async () => {
      ...
      const filename = await generateFilename(); // Use generated filename
      ...
      if(!jobCode) {
        showError('Please enter a job code');
        return;
      }
      
      // Check if PDFs already exist for this report ID
      const existingPDFs = await checkExistingPDFs(jid);
      let shouldOverwrite = false;
      
      if (existingPDFs.length > 0) {
        const existingCount = existingPDFs.length;
        const message = `${existingCount} PDF report${existingCount > 1 ? 's' : ''} already exist${existingCount > 1 ? '' : 's'} for this report.\n\nDo you want to replace the existing PDF${existingCount > 1 ? 's' : ''}?\n\n- OK: Replace existing PDF${existingCount > 1 ? 's' : ''}\n- Cancel: Abort PDF generation`;
        
        const userChoice = confirm(message);
        if (userChoice === false) {
          // User clicked Cancel - abort PDF generation entirely
          return;
        }
        shouldOverwrite = true; // User clicked OK - proceed with overwrite
      }
      ...
      try {
        ...
        await generatePDF(jid, reportNameEntry, items, uniqueFilename, isConfidential, jobName, shouldOverwrite);
  ...
  async function generatePDF(jid, reportNameEntry, items, filename, isConfidential, jobName, shouldOverwrite = false) {
    ...
    try {
      if (window.IndexedStorage && window.IndexedStorage.Storage) {
        const settingsData = await window.IndexedStorage.Storage.getItem("JL_settings_v3");
    ...
    // Merge MSAL user data with optional settings
    if (userData) {
      settings.name = userData.name;
      settings.mail = userData.email;
    }
    ...
    async function generatePDFWithLogos(headerLogoData, titleLogoData) {
      ...
      await generatePDFWithPinImages(headerLogoData, titleLogoData, pinImages, shouldOverwrite);
    }
    
    async function generatePDFWithPinImages(headerLogoData, titleLogoData, pinImages, shouldOverwrite = false) {
    ...
    for (let i = 0; i < orderedItems.length; i++) {
      ...
      if(it.type === "note") {
        ...
        noteLetterIndex++;
        continue; // Skip the rest of the image rendering logic for this iteration - NOTE CARDS SHOULD NOT INCREMENT imageCounter
      }
      ...
      // Increment image counter for next image item
      // (Note: this should only execute for image items due to continue statement above, 
      //  but adding explicit check as defensive programming)
      if (it.type !== "note") {
        imageCounter++;
      }
    }
    ...
    try {
      ...
      await createPDFCard(jid, cleanFilename, pdfBlobUrl, pdfDataUrl, finalTotalPages, reportId, jobName, shouldOverwrite);
  ...
  async function createPDFCard(jid, filename, pdfBlobUrl, pdfDataUrl, pageCount, reportId, jobName = null, shouldOverwrite = false) {
    console.log(`createPDFCard called with jid: ${jid}, filename: ${filename}, pageCount: ${pageCount}, reportId: ${reportId}, jobName: ${jobName}, shouldOverwrite: ${shouldOverwrite}`);
    ...
    if (shouldOverwrite) {
      // Remove all existing PDFs for this report ID and replace with the new one
      const otherPDFs = pdfs.filter(pdf => pdf.reportId !== jid);
      const finalPDFs = [...otherPDFs, pdfCard];
      await savePDFs(finalPDFs);
      console.log(`Overwrite mode: Replaced existing PDFs for report ${jid}. Total PDFs: ${finalPDFs.length}`);
    } else {
      // Add new PDF to existing ones
      pdfs.push(pdfCard);
      await savePDFs(pdfs);
      console.log(`New PDF mode: Added new PDF. Total PDFs: ${pdfs.length}`);
    }
  ...
  async function deleteReportAndJobName(reportId){
    ...
    if(wasCurrentReport) {
      ...
      if(ids.length > 0) {
        ...
        // Load report-specific data for the auto-selected report
        await loadReportDataToUI(ids[0].id);
      } else {
        ...
        // Clear report-specific data when no Report ID is selected
        await loadReportDataToUI("");
  ...
  async function deleteJobIdInternal(reportId){
    ...
    // Remove Job Name for this Report ID
    try{ 
      localStorage.removeItem(K_JOBNAME + reportId + "_v1");
      if (window.IndexedStorage && window.IndexedStorage.Storage) {
        window.IndexedStorage.Storage.removeItem(K_JOBNAME + reportId + "_v1").catch(console.warn);
      }
    }catch(_){}
  ...
  on(UI.btnClearData, "click", async function(){
    ...
    const confirmMessage = `⚠️ EMERGENCY RESET WARNING! ⚠️\n\nTHIS WILL DELETE EVERYTHING!\n\n` +
    ...
    if (confirm(confirmMessage)) {
      try {
        ...
        alert(`NUCLEAR RESET COMPLETE!\n\nApp has been wiped clean and is now back to factory state.`);
        ...
        // Clear report-specific data after clearing all data
        await loadReportDataToUI("");
  ...
  on(UI.btnCreate, "click", async function(){
    ...
    // Load report-specific data for new Report ID (will be empty but ensures clean state)
    await loadReportDataToUI(reportId);
  ...
  // Save Job Name when it changes
  const jobNameInput = document.getElementById('job-name');
  if (jobNameInput) {
    jobNameInput.addEventListener('input', async function() {
      const currentReportId = getCur();
      if (currentReportId) {
        await saveJobName(currentReportId, jobNameInput.value.trim());
      }
    });
  }
  ...
  if (locateBtn) {
    locateBtn.addEventListener('click', async function() {
      ...
      // Check permissions first and log detailed browser info
      try {
      ...
      for (let i = 0; i < attempts.length; i++) {
        try {
          ...
          if (i === attempts.length - 1) {
            // If we have a best position from earlier attempts, use it despite the final error
            if (bestPosition) {
              console.log(`Using best position despite final error - accuracy: ${Math.round(bestPosition.coords.accuracy)}m`);
              position = bestPosition;
              break;
            }
      ...
      // If we got here, we have a position
      if (position) {
        ...
        try {
          locateBtn.textContent = '⏳ Getting address...';
          const address = await reverseGeocodeEnhanced(lat, lng);
          
          // Enhanced accuracy-based messaging
          let message = '';
          let alertMessage = '';
          ...
          if (accuracy <= 50) {
            ...
            message = `✅ Excellent GPS accuracy (${Math.round(accuracy)}m)`;
          } else if (accuracy <= 200) {
            // Good accuracy with verification suggestion
            locationInput.value = `${address} (±${Math.round(accuracy)}m)`;
            message = `⚠️ Good GPS accuracy (${Math.round(accuracy)}m). Please verify the address.`;
            alertMessage = `GPS accuracy is ${Math.round(accuracy)} meters. Please verify the address is correct and edit if needed.`;
          } else if (accuracy <= 1000) {
            // Poor accuracy - needs verification
            locationInput.value = `${address} (±${Math.round(accuracy)}m - VERIFY REQUIRED)`;
            message = `⚠️ Poor GPS accuracy (${Math.round(accuracy)}m). Address needs verification.`;
            alertMessage = `⚠️ GPS accuracy is only ${Math.round(accuracy)} meters (${(accuracy/1000).toFixed(1)}km). This is likely cell tower positioning.\n\nFor better accuracy:\n• Go outdoors with clear sky view\n• Wait a few minutes for GPS satellites to lock\n• Try again when not indoors\n\nPlease verify and edit the address as needed.`;
          } else {
            // Very poor accuracy - basically useless
            locationInput.value = `${address} (±${Math.round(accuracy)}m - VERY INACCURATE)`;
            message = `❌ Very poor GPS accuracy (${(accuracy/1000).toFixed(1)}km). Manual entry recommended.`;
            alertMessage = `❌ GPS accuracy is extremely poor: ${(accuracy/1000).toFixed(1)} kilometers!\n\nThis is cell tower triangulation, not actual GPS. The address may be completely wrong.\n\nRecommendation:\n• Go outside with clear view of sky\n• Wait 2-3 minutes for GPS satellite lock\n• Try again, or enter address manually\n\nPlease verify this address is correct or enter manually.`;
          }
  ...
  window.addEventListener("load", async function(){
    ...
    // Load report-specific data for current Report ID (job code, location)
    await loadReportDataToUI(jid || "");
    
    // Update UI controls state now that report ID is loaded
    if (window.updateUIControlsState) {
      await window.updateUIControlsState();
      console.log('Updated UI controls state after page load with report ID:', jid);
    }
